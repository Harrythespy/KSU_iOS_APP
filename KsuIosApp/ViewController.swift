...
class ViewController: BaseController {
    ...
    @IBOutlet weak var usr: UITextField! //UI的ksuid輸入框
    ...
    override func viewDidLoad() {
        ...
        //使用輸入框時出現刪除按鈕
        usr.clearButtonMode = .whileEditing
    ...
    func postData() {
        ...
        let postString = "acc=\(usr.text!)&pas=\(pwd.text!)" //將兩個輸入框的值儲存為字串
        ...
        let task = URLSession.shared.dataTask(with: request as URLRequest) { (data, response, error) in
            ...
            if let httpStatus = response as? HTTPURLResponse, httpStatus.statusCode != 200 {           // check for http errors
                ...
                self.m.alertMessage(UIViewController: self, alertTitle: "伺服器發生錯誤", alertMessage: "請稍候重新嘗試登入")
            }
            else {
                ...
                do { //json解析時的拋出事件
                    ...
                    self.ksuId = userJson.ksuId
                    self.studentId = userJson.studentId
    ...
    func tryLogin() {
        ...
        //這個區塊是判斷輸入框是否都有輸入
        if (self.usr.text == "" || self.pwd.text == "") { //如果輸入框沒有輸入任何值
        ...
        //儲存使用者的ksuid
        defaults.set(self.ksuId, forKey: "ksuId")
        //儲存使用者的student id
        defaults.set(self.studentId, forKey: "studentId")
        //儲存使用者的名字
